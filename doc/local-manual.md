...
Firebase Emulator を起動した状態で、別のターミナルから次を実行します。

```bash
make mocks
```

イベント「動作確認テスト」（2000年）に、次のデータが登録されます。
...
フォームの項目、セクション名、学年、氏名の全角スペースなどは、本番環境で使用する入力形式に合わせています。既に2000年のモックデータがある場合は、再登録によって確認用データを同じ状態に戻せます。

モックデータを削除する場合は次を実行します。

```bash
make mocks/del
```

2000年に関連するイベント、フォーム、回答、配布班、配布区域、割り当て、店舗データが削除されます。
...
| make mocks     | Firebase Emulatorに2000年の動作確認用データを登録する。           |
| make mocks/del | 2000年の動作確認用データを関連データごと削除する。                |
